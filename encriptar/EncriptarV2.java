...
public class EncriptarV2 {
  ...
  public static void encriptarFichero (String fileRead, String fileWrite) {
    ...
    // a partir de aquí varía respecto a mi original.
    try {
      ...
      for (int i=0; i < posicion-1;i++) {
        String caracterABuscar = lectura.substring(i,i+1);
        String cadenaEncriptada = "";
        // Si el carácter se encuentra en nuestro diccionario, encriptamos.
        if (diccionarioPrincipal.contains(caracterABuscar)){
          int posicionDondeEsta = diccionarioPrincipal.indexOf(caracterABuscar);
          int posicionCaracterEncriptado = ((posicionDondeEsta + clave) % lectura.length());
          caracterABuscar = lectura.substring(posicionCaracterEncriptado, posicionCaracterEncriptado+1);
        }
        escritura.append(caracterABuscar);        
      }
